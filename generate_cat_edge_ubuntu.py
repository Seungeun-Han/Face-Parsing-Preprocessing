...
image_list = [x[:-4].zfill(5) for x in image_list if int(x[:-4]) >= 10000 and int(x[:-4]) < 20000]
...
saved_subject_list = [x[:5] for x in saved_list]

image_list = list(set(image_list) - set(saved_subject_list))
print("The # of left images: ", len(image_list))
#print(image_list)
...
for im_list in image_list:
    ...
    for idx, ann_list in enumerate(annotation_list):
        if im_list in ann_list:
            ...
            cv2.imwrite(save_dir + im_list + "_" + ann_list[6:-4] + ".png", edge)
