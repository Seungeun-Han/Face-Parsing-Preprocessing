...
LABELS = {'bg': 0, 'skin': 1, 'nose': 2, 'eye_g': 3, 'l_eye': 4, 'r_eye': 5,
        'l_brow': 6, 'r_brow': 7, 'l_ear': 8, 'r_ear': 9, 'mouth': 10, 'u_lip': 11,
        'l_lip': 12, 'hair': 13, 'hat': 14, 'ear_r': 15, 'neck': 16, 'cloth': 17, 'neck_l': 18, 'hand': 20}  # mask가 19
...
def make_groundTruth(label, seg, seg_name):
    ...
    * 우선순위 중요함
    ...
    assert_list = ['skin', 'l_ear', 'r_ear', 'l_brow', 'r_brow', 'neck', 'l_eye', 'r_eye', 'hair', 'nose', 'mouth', 'u_lip', 'l_lip', 'hat', 'neck_l']
    ...
    if seg_name not in assert_list:
        ...
        ear_idx = [x for x in range(len(label[0])) if label[0][x] != LABELS['hand'] and label[0][x] != LABELS['hair'] and label[0][x] != LABELS['ear_r']]  # *10
        intersection = list(set(seg_list) & set(ear_idx))
        label[0][intersection] = LABELS[seg_name]# * 10
    elif seg_name == 'l_brow' or seg_name == 'r_brow':  # cond3, 7
        brow_idx = [x for x in range(len(label[0])) if label[0][x] != LABELS['hand'] and label[0][x] != LABELS['hair'] and label[0][x] != LABELS['hat']]  # *10
        intersection = list(set(seg_list) & set(brow_idx))
        label[0][intersection] = LABELS[seg_name]# * 10
    elif seg_name == 'l_eye' or seg_name == 'r_eye':  # cond4
        eye_idx = [x for x in range(len(label[0])) if label[0][x] != LABELS['hand'] and label[0][x] != LABELS['hair']]  # *10
        intersection = list(set(seg_list) & set(eye_idx))
        label[0][intersection] = LABELS[seg_name] #* 10
    elif seg_name == 'neck':  # cond5
        neck_idx = [x for x in range(len(label[0])) if label[0][x] != LABELS['hand'] and label[0][x] != LABELS['ear_r']]  # *10
        ...
        hair_idx = [x for x in range(len(label[0])) if label[0][x] != LABELS['hand'] and label[0][x] != LABELS['hair'] and label[0][x] != LABELS['eye_g']]  # *10
        intersection = list(set(seg_list) & set(hair_idx))
        label[0][intersection] = LABELS[seg_name] #* 10
    elif seg_name == 'mouth' or seg_name == 'u_lip' or seg_name == 'l_lip':  # cond8
...
# image_list = [i[:5]+".jpg" for i in annotation_list if "hat" in i]
annotation_name_list = [i[:5] for i in annotation_list]
annotation_name_list = list(set(annotation_name_list))

save_dir = r"D:\Dataset\CelebAMask-HQ-hand/CelebAMask-HQ-hand_256/labels_hand0/" #fast_labels
...
for im_list in image_list:
    ...
    seg_list = [i for i in range(len(label[0])) if label[0][i] == LABELS['hand']]
