...
LABELS = {'bg': 0, 'skin': 1, 'nose': 2, 'l_brow': 3, 'r_brow': 4, 'eye_g': 5,
        'l_eye': 6, 'r_eye': 7, 'mouth': 8, 'u_lip': 9, 'l_lip': 10, 'l_ear': 11,
        'r_ear': 12, 'hair': 13, 'ear_r': 14, 'hat': 15, 'neck': 16, 'neck_l': 17, 'cloth': 18, 'mask': 19}
...
# image_list = [x for x in image_list if int(x[:-4]) < 5000] #  >= 10000 and int(x[:-4])
...
save_dir = "D:/Dataset/CelebAMask-HQ/Nsense_CelebA-HQ_512/labels/"
...
for im_list in image_list:
    ...
    if parent_img_name in annotation_name_list:
        ...
        for p in part_list:
            ...
            parsing_anno = cv2.resize(parsing_anno, (512, 512), cv2.INTER_NEAREST)
