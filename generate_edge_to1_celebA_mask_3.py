...
im_path = os.path.join('D:/Dataset/CelebAMask-HQ-mask/images_5blanks/')
image_list = os.listdir(im_path)
image_list = [i for i in image_list if int(i[:5]) >= 20000 and int(i[:5]) < 30000]

parsing_anno_path = os.path.join('D:/Dataset/CelebAMask-HQ-mask/seg_1024png/')
annotation_list = os.listdir(parsing_anno_path)
annotation_list = [i for i in annotation_list if int(i[:5]) >= 20000 and int(i[:5]) < 30000]
annotation_name_list = [i[:5] for i in annotation_list]
annotation_name_list = list(set(annotation_name_list))

save_dir = "D:/Dataset/CelebAMask-HQ-mask/edges/"
